...
export const visaWorkflows: readonly VisaWorkflow[] = [
  ...
  },
  {
    ...
    description:
      "Check whether an eligible passport can enter without an advance visa.",
    requiredProfile: [
      "nationality",
      "passport type",
      "travel purpose",
  ...
  },
  {
    ...
    description:
      "Understand registration after entry and accommodation responsibilities.",
...
const workflowTerms: readonly [VisaWorkflowId, readonly string[]][] = [
  [
    "overstay-and-exit",
    ["overstay", "expired", "exit visa", "просроч", "muddati tug"],
  ],
  [
    "arrival-registration",
    ["register", "registration", "hotel registration", "регистрац", "ro'yxat"],
  ],
  [
    "residence-permit",
    [
  ...
  [
    "work-visa",
    ["work visa", "work permit", "employment", "job", "работ", "ish"],
  ],
  [
    "student-visa",
    ["student", "study", "university", "school", "учеб", "o'qish"],
  ],
  [
    "family-visit",
    ["family", "private visit", "pv-1", "pv-2", "vtd", "relative", "семь"],
  ],
  [
    "business-visa",
    [
...
export function selectVisaWorkflow(question: string): VisaWorkflow {
  ...
  const selectedId =
    workflowTerms.find(([, terms]) =>
      terms.some((term) => normalized.includes(term)),
    )?.[0] ?? "visa-route-discovery";
  return (
    visaWorkflows.find((workflow) => workflow.id === selectedId) ??
    visaWorkflows[0]
  );
}
...
export function evidenceForWorkflow(
  ...
  const candidates = visaKnowledge.chunks
    ...
    .map((chunk) => {
      ...
      const topicPriority =
        workflow.evidenceTopics.length -
        workflow.evidenceTopics.indexOf(chunk.topic);
  ...
  for (const { chunk } of candidates) {
    ...
    if (selected.has(chunk.id) || (perSource.get(chunk.sourceFile) ?? 0) >= 2)
      continue;
