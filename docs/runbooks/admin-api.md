...
Read-only source and job queries require PostgreSQL and the environment-bound registry only. They remain available when Redis or evidence storage is degraded so administrators can inspect state and errors. Crawler enqueue and document upload continue to fail closed until their queue and object-store dependencies are configured.
