...
The internal ingestion operations dashboard is available at `http://localhost:3000/admin`, with the reviewer console at `http://localhost:3000/admin/reviews`. The dashboard lists the configured source registry, lets administrators add audited manual-only official sources, accepts bounded PDF and JSON evidence uploads assigned to an existing or newly named topic, converts readable PDFs to page-preserving Markdown and review JSON, queues registry-approved crawlers, and displays recent job health. Admin-created sources cannot enable crawling, and uploaded evidence remains unavailable to the assistant until review and publication. Both surfaces require an authorized staff principal.
