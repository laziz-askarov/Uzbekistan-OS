...
> queue persistence, role-gated review transitions, artifact comparison,
...
**Current progress:** the complete non-production source-to-index core is implemented: registry validation and synchronization, deterministic scheduling, safe fetch/snapshot/extraction for HTML/text/PDF, change detection, Redis delivery, database-authoritative retry/dead-letter state, S3-compatible evidence, review queue and comparison, fail-closed identity/roles, evidence-bound publication, immutable versions/audit/lifecycle events, semantic chunking with citation provenance, publisher publish/expire/re-index API and UI controls, provider-neutral embedding/index processing with idempotency/retries/vector validation/cost-latency telemetry, and live PostgreSQL retrieval-eligibility evidence. An inert five-source production proposal and 21-document multilingual inventory are checked in. Phase closure still requires approved production sources and source-specific adapters, an approved token verifier, D-004/D-005/D-006 decisions, and 20 independently reviewed production documents.
...
**Current progress:** the internal retrieval and grounded-orchestration cores are implemented: typed deterministic language/intent/risk planning for all 15 launch flows, strict trust/applicability filters, eligibility-view lexical and exact-vector queries with current official-source checks, reciprocal-rank fusion, bounded citation-preserving evidence packs, retrieved-control-pattern quarantine, immutable layered prompt versions and fingerprints, provider-neutral approved routes with timeout/retry/token/cost/non-storage controls, a strict grounded-answer schema, claim-level evidence and exact-quote validation, risk-sensitive lexical support checks, and localized fail-closed insufficiency behavior. PRD-aligned runtime settings validate the five domains, three languages, official-source/citation invariants, evidence and conversation-context budgets, 2s/3s/8s response targets, a 95% benchmark citation target, and a disabled proposed balanced generation route. The conversation-context core now bounds recent turns, excludes deleted/control-pattern content, validates exact-quote-cited summaries, prevents summary overlap, fingerprints accepted state, and explicitly prevents memory from serving as official evidence. The frozen evaluation contract now covers all 15 workflows in English, Uzbek, and Russian with 45 golden, adversarial, and abstention cases; it calculates retrieval, citation, safety, latency, and cost metrics and distinguishes fail from prerequisite-blocked results. The deterministic planning baseline passes 15/15. Public search/chat, the production provider adapter and approved D-006 route, production embeddings and ANN indexing, conversation persistence/summary generation, approved source labels, and live retrieval/generation gate evidence remain incomplete.
